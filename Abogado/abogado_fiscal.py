...
if "juego" in st.query_params:
    ...
    # Preparamos los emojis para mostrarlos en el menú
    ...
    <div id="pantalla-inicio" class="pantalla activa">
        ...
        <p style="font-size: 18px; color: #e74c3c; font-weight: bold;">⚠️ ¡Cuidado! La IA enemiga aprenderá tus movimientos y te atacará donde más te escondas.</p>
        <button class="btn" onclick="iniciarJuego()">¡Empezar Juicio!</button>
    </div>

    <div id="pantalla-juego" class="pantalla" style="text-align: center;">
        <canvas id="game-canvas" width="650" height="450"></canvas>
    </div>

    <div id="pantalla-victoria" class="pantalla">
        <h1 style="color: #2ecc71; font-size: 45px; margin-top: 0;">¡ORDEN EN LA SALA! 💥</h1>
        <div class="caja-sentencia">
            <h2 style="color: #2ecc71; margin-top: 0; border-bottom: 2px solid #2ecc71; padding-bottom: 10px;">📜 VEREDICTO OFICIAL:</h2>
            <div style="font-size: 20px; line-height: 1.6; color: #ecf0f1;">{sentencia_limpia}</div>
        </div>
    </div>
    ...
    <script>
        ...
        let score = 0;
        let timeLeft = 20; 
        let gameLoop, timerLoop, spawnLoop;
        let isPlaying = false;
        ...
        canvas.addEventListener('mousemove', function(evt) {{
            ...
            if(isPlaying) {{
                historialX.push(player.x);
                if(historialX.length > 50) historialX.shift(); 
            }}
        }});
        ...
        function spawnItem() {{
            ...
            const textoRaw = (listaTextos && listaTextos.length > 0) ? listaTextos[Math.floor(Math.random() * listaTextos.length)] : (esBueno ? "💎" : "💩");
            const textoRandom = textoRaw.toUpperCase();
            
            let spawnX = Math.random() * (canvas.width - 100); 

            if (!esBueno && historialX.length > 20) {{
                let promedioX = historialX.reduce((a, b) => a + b, 0) / historialX.length;
                if (Math.random() < 0.75) {{
                    spawnX = promedioX + (Math.random() * 60 - 30); 
                }}
            }}
        ...
        function actualizarYDibujar() {{
            ...
            ctx.fillText("Tiempo: " + timeLeft + "s", canvas.width - 20, 40);
            ...
            for (let i = 0; i < items.length; i++) {{
                ...
                // DIBUJAR EMOJI GRANDE Y LIMPIO (Rendimiento máximo)
                ctx.font = "50px Arial"; 
                ctx.textAlign = "center";
                ctx.textBaseline = "middle";
                ctx.fillText(item.texto, 0, 0);
                
                ctx.restore();

                // Colisiones
                if (item.x + item.width/2 > player.x &&
                    item.x + item.width/2 < player.x + player.width &&
                    item.y + item.height/2 > player.y &&
                    item.y + item.height/2 < player.y + player.height) {{
                    
                    if (item.bueno) score += 15;
                    else score -= 10;
        ...
        function iniciarJuego() {{
            ...
            actualizarYDibujar();
            spawnLoop = setInterval(spawnItem, 350); 
        ...
        function finalizarJuego() {{
            ...
            if (score >= 50) cambiarPantalla('pantalla-victoria');
                ...
                document.getElementById('puntos-finales').innerText = "Puntos conseguidos: " + score;
                cambiarPantalla('pantalla-derrota');
            }}
        }}
...
def generador_texto(flujo):
    for trocito in flujo:
        yield trocito['message']['content']
...
if st.session_state.juicio_iniciado:
    ...
    # Re-dibujar toda la conversación guardada en memoria
    for personaje, mensaje in st.session_state.ui_chat:
        avatar = "🧐" if personaje == "Fiscal" else "👨‍⚖️"
        with st.chat_message(personaje, avatar=avatar):
            st.write(mensaje)
            
    # --- CORRECCIÓN: Mostrar el gráfico SOLO si ya han hablado (ronda > 0) ---
    if st.session_state.ronda_actual > 0:
        st.markdown(f"**📈 Puntuación Ronda {st.session_state.ronda_actual}:**")
    ...
    if st.session_state.ronda_actual < RONDAS:
        if st.button("🗣️ Siguiente Turno"):
    ...
    # Final del Juicio (Resolución)
        ...
        if st.button("👨‍⚖️ Dictar Sentencia Final", type="primary"):
            with st.spinner('Redactando la sentencia y generando el minijuego...'):
                
                # Rescatamos las últimas respuestas para el juez
                resp_f_final = st.session_state.ui_chat[-2][1]
                resp_a_final = st.session_state.ui_chat[-1][1]
                ...
                # --- OPCIÓN 1: IA DIRECTOR DE ARTE (EMOJIS) ---
                sys_palabras = """Eres un director de arte para un videojuego. Tu trabajo es elegir emojis visuales que representen la compra absurda que se debate hoy.
                No escribas palabras, solo emojis. No generes introducciones ni conclusiones.
                Responde ÚNICAMENTE con una lista de EXACTAMENTE 6 emojis separados por comas. Los primeros 3 deben ser visuales positivos/graciosos sobre este objeto. Los segundos 3 deben ser visuales negativos/críticos.
                Ejemplo de respuesta si el objeto es un 'coche': 🚗,🚀,✨,💥,💸,📉. No añadas nada más."""
                ...
                try:
                    ...
                    texto_palabras = res_palabras['message']['content'].strip()
                    
                    emojis_lista = [e.strip() for e in texto_palabras.split(',') if e.strip()]
                    
                    if len(emojis_lista) >= 6:
                        palabras_ok = emojis_lista[0:3]
                        palabras_mal = emojis_lista[3:6]
                    else:
                        raise ValueError("La IA no generó suficientes emojis")
                        
                except Exception as e:
                    palabras_ok = ["🌟", "👍", "🥳", "✨", "💎", "🏆"] 
                    palabras_mal = ["🤮", "💀", "💸", "🗑️", "💩", "📉"]

                if not palabras_ok or not palabras_mal:
                     palabras_ok = ["💎", "✨", "🏆"] 
                     palabras_mal = ["💩", "💸", "🗑️"]
                     
                # Guardamos todo para el juego
                datos_para_juego = {
                    "objeto": st.session_state.compra,
                    "juez": respuesta_juez,
                    "palabras_ok": palabras_ok,
                    "palabras_mal": palabras_mal
                }
                with open("juicio_actual.json", "w", encoding="utf-8") as archivo:
                    json.dump(datos_para_juego, archivo, ensure_ascii=False)

                # GENERADOR PDF
                def crear_pdf(objeto, sentencia):
                    pdf = FPDF()
                    pdf.add_page()
                    ...
                    objeto_limpio = objeto.encode('latin-1', 'replace').decode('latin-1')
                    pdf.cell(200, 10, txt=f"OBJETO JUZGADO: {objeto_limpio}", ln=True, align='L')
                    pdf.ln(5)
                    pdf.cell(200, 10, txt="--- VEREDICTO FINAL ---", ln=True, align='L')
                    pdf.set_font("Arial", '', 12)
                    sentencia_limpia = sentencia.encode('latin-1', 'replace').decode('latin-1')
                    pdf.multi_cell(0, 10, txt=sentencia_limpia)
                    pdf.ln(15)
                    url_local = "http://localhost:8501/?juego=true" 
                    pdf.set_text_color(0, 0, 255) 
                    pdf.set_font("Arial", 'U', 12) 
                    pdf.cell(0, 10, txt=">> HAZ CLIC AQUI PARA EJECUTAR LA SENTENCIA (MINIJUEGO) <<", ln=True, align='C', link=url_local)
                    return pdf.output(dest='S').encode('latin-1')

                pdf_bytes = crear_pdf(st.session_state.compra, respuesta_juez)
                
                # --- EL SECRETARIO JUDICIAL (IA) REDACTA EL ACTA DE CONCLUSIONES ---
                ...
                Tu tarea es leer un debate completo y redactar el Acta Oficial de Conclusiones.
                Tu respuesta DEBE ser el documento final estructurado en Markdown. 
                Debes incluir:
                1. Un título creativo para el caso.
                2. Un resumen inteligente de los argumentos del Fiscal.
                3. Un resumen inteligente de los argumentos del Abogado.
                4. Las conclusiones finales basadas en el veredicto del Juez.
                5. MUY IMPORTANTE: Al final del documento, despídete y añade EXACTAMENTE este enlace en formato Markdown para que el usuario pueda cumplir la condena: [👉 HAZ CLIC AQUÍ PARA EJECUTAR LA SENTENCIA (MINIJUEGO)]({url_juego})
                
                NO me hables a mí, genera SOLO el texto del documento."""

                contexto_juicio = f"Objeto: {st.session_state.compra}\nFiscal: '{resp_f_final}'\nAbogado: '{resp_a_final}'\nJuez: '{respuesta_juez}'"
                
                res_secretario = ollama.chat(model=MODELO_JUEZ, messages=[
                    {"role": "system", "content": sys_secretario}, 
                    {"role": "user", "content": f"Por favor, redacta el archivo de conclusiones para este caso:\n\n{contexto_juicio}"}
                ])
                
                acta_ia = res_secretario['message']['content']
                acta_bytes = acta_ia.encode('utf-8')
    ...
    if st.session_state.get("juicio_terminado", False):
        ...
        st.info("Descarga el archivo PDF o las conclusiones. Dentro encontrarás el enlace para jugar.")
        ...
        st.download_button(label="📝 Descargar Conclusiones de la IA (.MD)", data=st.session_state.acta_bytes, file_name="Conclusiones_IA.md", mime="text/markdown")
