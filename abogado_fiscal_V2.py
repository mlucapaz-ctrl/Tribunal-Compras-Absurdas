        ...
        if st.session_state.ronda_actual < RONDAS:
            ...
            if st.button("👨‍⚖️ Dictar Sentencia Final", type="primary"):
                with st.spinner('Redactando la sentencia y generando la portada del periódico...'):
                    ...
                    # 5. GENERAR ACTA MD (Ahora incluye la noticia pegada arriba del todo)
                    url_juego = "http://localhost:8501/?juego=true"
                    sys_secretario = f"""Resume el juicio en Markdown de forma estructurada. Despídete con este enlace al final: [👉 EJECUTAR SENTENCIA]({url_juego})"""
                    res_secretario = ollama.chat(model=MODELO_JUEZ, messages=[{"role": "system", "content": sys_secretario}, {"role": "user", "content": f"Juez: '{respuesta_juez}'"}])
