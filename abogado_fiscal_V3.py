"""En esta nueva versión respecto a la anterior, he añadido un "Testigo Experto" (con Qwen 2.5:3b) del cual es experto en la compra que se debata, este testigo es llamado por el abogado para ayudarle a defender la compra a muerte, y también es llamado por el juez para que le ayude a redactar la sentencia final. He utilizado a Qwen ya que era la IA que mejor se adaptaba a este rol de "Asesor Especialista" (en comparación con los otros modelos más generalistas) y también era el que menos participaba (solo con los emojis). """
...
if st.session_state.juicio_iniciado:
    ...
    for item in st.session_state.ui_chat:
        ...
        elif tipo in ["Fiscal", "Abogado", "Testigo"]:
            if tipo == "Fiscal": avatar = "🧐"
            elif tipo == "Abogado": avatar = "👨‍⚖️"
            else: avatar = "👨‍🔬"
    ...
    if st.session_state.get("turno_pendiente", False):
        ...
        # 🟢 RONDA 1: FISCAL VS ABOGADO (Normal)
        ...
        # 🟢 RONDA 2: TESTIGO EXPERTO (QWEN) -> FISCAL -> ABOGADO
        elif st.session_state.ronda_actual == 2:
            with st.chat_message("Testigo", avatar="👨‍🔬"):
                st.info("⚠️ El Abogado ha llamado a declarar a un Testigo Experto sorpresa...")
                sys_testigo = f"Eres un experto mundial absurdamente especializado en '{st.session_state.compra}'. Inventa tu nombre y un título universitario falso y rimbombante. Defiende que este objeto es una revolución indispensable. Tono pedante y loco. Máximo 4 líneas."
                
                flujo_testigo = ollama.chat(model=MODELO_GUIONISTA, messages=[{"role": "system", "content": sys_testigo}, {"role": "user", "content": "Por favor, dé su testimonio."}], stream=True)
                respuesta_testigo = st.write_stream(generador_texto(flujo_testigo))
                st.session_state.ui_chat.append(("Testigo", respuesta_testigo))
                
                st.session_state.historial_fiscal.append({"role": "user", "content": f"¡PROTESTO! El abogado ha traído un testigo experto que afirma: '{respuesta_testigo}'. ¡Destrúyelo en tu interrogatorio!"})
                st.session_state.puntos_abogado += random.randint(10, 20)
            ...
            with st.chat_message("Fiscal", avatar="🧐"):
                ...
                st.session_state.historial_abogado.append({"role": "user", "content": f"El fiscal acaba de atacar a tu testigo diciendo: '{respuesta_fiscal}'. Haz tu alegato final defendiendo la compra."})
                st.session_state.ui_chat.append(("Fiscal", respuesta_fiscal))
                st.session_state.puntos_fiscal += random.randint(10, 30)
    ...
    # --- BOTONES DE CONTROL ---
        ...
        if st.session_state.ronda_actual < RONDAS:
            if st.button("🗣️ Siguiente Turno (Llamar al Testigo Experto)", type="primary"):
                st.session_state.ronda_actual += 1
